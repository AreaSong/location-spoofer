...
grep -q 'Location Spoofer CA' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "certificate setup must use the stable app-branded CA name"
! grep -q 'WLOC CA' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "certificate setup must not show the legacy dated CA name"
...
grep -q '重置证书' "$ROOT"/App/SettingsView.swift "$ROOT"/App/SettingsView+*.swift || fail "APP mode settings must expose certificate reset"
grep -q 'certificateStore.reset()' "$ROOT"/App/SettingsView.swift "$ROOT"/App/SettingsView+*.swift || fail "certificate reset must remove the persisted app CA"
grep -q 'requestCertificateSetup()' "$ROOT"/App/SettingsView.swift "$ROOT"/App/SettingsView+*.swift || fail "certificate reset must open the certificate setup flow"
grep -q 'requestThirdPartyOnboarding()' "$ROOT"/App/SettingsView.swift "$ROOT"/App/SettingsView+*.swift || fail "third-party settings must keep a manual setup-guide action"
grep -q 'SFSafariViewController' "$ROOT/App/SafariView.swift" || fail "certificate download must use an in-app Safari service"
grep -q 'prepareCertificateDownloadURL' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "certificate setup must prepare an in-app download URL"
...
grep -q 'AppModeWiFiProxy' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "APP proxy setup must show the Wi-Fi screenshot"
grep -q 'AppModeCertificateInstall' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "certificate install setup must show its screenshot"
grep -q 'AppModeCertificateTrust' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "certificate trust setup must show its screenshot"
grep -q 'UIImage(named: assetName)' "$ROOT"/App/FirstSetupView.swift "$ROOT"/App/FirstSetupView+*.swift || fail "missing screenshots must fall back to text without breaking setup"
