...
- 原生地图选点和位置场景切换；
- Apple 定位服务响应的测试环境模拟；
- 本地代理、第三方代理和开发者隧道三种运行模式；
- 路线播放：设定起点、途经点和终点后沿路移动，可保存复用；
- 坐标标准识别与 WGS-84 / GCJ-02 双坐标管理；
- 最近选点历史、收藏排序与备份导入导出；
- 主页运行状态（本机代理/保活、第三方模块连接，或开发者隧道就绪）；
- 随机扰动、定位精度，以及 APP 模式下的运动状态模拟；
- 环境检测、运行日志和问题诊断。
...
- **原生地图交互**
    ...
    - 主页显示当前地图坐标标准；选点同时给出国内坐标和国际坐标，可分别复制；
...
- **定位服务响应模拟**
    ...
    - 在测试环境中返回选定的坐标数据；
    - APP 模式和第三方代理模式均可配置随机扰动半径和定位精度，避免测试点始终落在同一坐标；
    - 运动状态模拟仅 APP 模式可用；
    - 不需要修改目标 App 代码。

- **路线播放**
    - 在地图上依次放下起点、途经点（最多 5 个）和终点，按步行或骑行沿路规划路径；
    - 可设置速度、偏移距离和一次 / 往返 / 循环；
    - 路线可保存、覆盖、倒着走，最多 20 条；
...
- **三种运行模式**
    ...
    - 开发者隧道模式：不拦截网络请求，通过本机隧道调用系统的开发者定位模拟接口，需要 iOS 18 及以上系统、LocalDevVPN 和一次电脑配对。

- **环境检测**
    - APP 模式检测本地代理、证书信任和请求链路；
    - 主页显示当前模式需要的运行状态：APP 模式为代理运行与保活是否正常，第三方模式为模块是否已连接，开发者隧道模式为隧道和配对文件是否就绪；点按该行打开设置；
...
- 启用测试位置后使用近不可闻音频保活，并用看门狗在播放中断后恢复，避免本机代理在约两分钟后停止处理请求。
...
- App 负责地图选点、最近选点、收藏备份、坐标同步和清除；
- 第三方客户端负责代理/VPN、MITM、证书和规则执行；
- 默认从本 App 提供模块和脚本（`127.0.0.1:18766`），导入或更新时请保持本 App 打开；
- 坐标持久化由第三方客户端负责；
- 是否支持 Wi-Fi、4G 或 5G 取决于客户端；
- App 关闭后，第三方客户端中的配置可能继续生效。从第三方代理模式切走前，以及切到 APP 模式前，会先清除已保存的 WLOC 坐标；清除失败则保持当前模式。离开开发者隧道模式前会先关闭系统模拟定位，关不掉则保持开发者隧道模式。
...
开发者隧道模式不拦截任何网络请求，也不需要代理、CA 或第三方模块。它通过设备本机的一条隧道，调用 iOS 提供给开发者
工具的定位模拟接口，把坐标直接推进系统定位。
...
地图选点 / 路线采样
  ...
  │ WGS-84 坐标
  ▼
LocalDevVPN 本机隧道（10.7.0.1）
  │
  │ RemotePairing 配对 + RSD 握手
  ▼
iOS 开发者定位模拟服务
  │
  ▼
...
- 需要 iOS 18 或更新的系统；
- 需要安装第三方 App [LocalDevVPN](https://apps.apple.com/us/app/localdevvpn/id6755608044) 并连上本机隧道；
- 需要用电脑生成一次 RemotePairing 配对文件（`.plist`），在 App 内导入后无需再连电脑；
- 配对文件保存在 App 沙盒中，只在首次解锁后可读，并且不进入 iCloud 或电脑备份；
- 定点和路线都通过这条通道写入，路线播放每秒推送一次采样；
- 退出路线或走完后会停在当前虚拟点并转成定点；停止虚拟定位才会清除模拟、恢复真实位置，不需要开关定位服务；
- 随机扰动和定位精度设置在此模式下不生效，路线偏移仍然生效；
- 隧道断开时 App 会提示重新连接，并暂停正在播放的路线。

隧道能力由 LocalDevVPN 提供，定位模拟由 iOS 系统服务执行，App 只负责配对、连接和推送坐标。
...
| 客户端 | 模块文件 | 导入 | 脚本存储 | HTTPS 解密 | WLOC 改写 | 真机状态 | 备注 |
|---|---|---|---|---|---|---|---|
| Shadowrocket | wloc.module | 已提供 | 已提供 | 依赖客户端 | 已提供 | 已在真机验证 | 当前用于真机测试 |
| Surge | wloc.sgmodule | 已提供 | 已提供 | 依赖客户端 | 已提供 | 尚未真机验证 | 配置已提供 |
| Quantumult X | wloc.conf | 已提供 | 已提供 | 依赖客户端 | 已提供 | 尚未真机验证 | 配置已提供 |
| Loon | wloc.lpx | 已提供 | 已提供 | 依赖客户端 | 已提供 | 尚未真机验证 | 配置已提供 |
| Stash | wloc.stoverride | 已提供 | 已提供 | 依赖客户端 | 已提供 | 尚未真机验证 | 配置已提供 |
| Egern | wloc.sgmodule | 已提供 | 已提供 | 依赖客户端 | 已提供 | 尚未真机验证 | 使用 Surge 模块，配置已提供 |

iOS 27 beta 6 起，系统禁止对 `gs-loc.apple.com` 做 MITM，上表全部客户端都暂时不可用。未标成「已在真机验证」的客户端只表示模块文件已提供，不代表真机已经跑通。
...
- iOS 18 及以上、无法再对 Apple 定位服务做 MITM 的系统；
- 需要路线连续移动、且不想反复开关定位服务的测试；
- 不希望修改 Wi-Fi 代理或安装证书的场景。

使用条件：

- iOS 18 或更新的 iOS 设备；
- 已安装 LocalDevVPN 并连上本机隧道；
- 已用电脑生成并导入一次 RemotePairing 配对文件；
- 在 App 内完成开发者隧道引导。
...
1. 选择 APP 模式、第三方代理模式或开发者隧道模式；
2. 按照 App 内引导完成对应配置；
3. 执行环境检测；
4. 在地图中搜索、粘贴链接、输入坐标、点击或拖动选择测试位置；可用最近选点和收藏快速回显；
5. 启用测试位置并在目标测试环境中验证结果；
6. 需要移动时切到底部的“走路”，放下起点和终点后开始播放。
...
1. 停止测试位置（停止虚拟定位），App 会立即清除系统定位模拟；退出路线或走完会留在当前虚拟点；
2. 如需彻底断开，在 LocalDevVPN 中关闭隧道。
...
标准，并在运行期间根据蓝点变化和用户操作进行受控刷新。固定锚点名称只有命中白名单才会判定为对应标准；
英文系统下未列入白名单的名称不会把国内地图误判为 WGS-84，从而避免约 200–500 米的偏移。

主页和设置都会显示当前地图坐标标准。搜索框支持地点名称、纯经纬度，以及苹果、谷歌和高德地图链接。纯经
纬度会记住上次选用的标准；地图链接按来源推断标准。
...
Shared/     坐标、收藏、最近选点、路线、日志、配置和共享模型
Vendor/     开发者隧道使用的 idevice 静态库及来源说明
...
开发者隧道模式链接的 idevice 静态库以预编译文件的形式放在 `Vendor/idevice/`，来源和重新构建方式见该目录下的说明。
...
- 开发者隧道模式只连接本机隧道地址（10.7.0.1），配对文件受文件保护且不进入备份；
- App 生成的 CA 私钥保存在设备 Keychain 中；
- 第三方客户端模块、MITM 和证书链路由用户选择的客户端负责。

请不要把真实位置、认证信息、证书私钥、配对文件或完整敏感日志提交到公开 Issue。

## 限制

- iOS 系统版本变化可能影响定位服务行为；
- iOS 27 beta 6 起系统禁止对 `gs-loc.apple.com` 做 MITM，APP 模式和第三方代理模式在这些版本上不可用；
- 开发者隧道模式需要 iOS 18 及以上系统，并依赖第三方 App LocalDevVPN 提供隧道；
