...
- After a test location is enabled, near-inaudible audio keep-alive plus a watchdog restart playback if it stops, so the
  on-device proxy does not stop handling requests after about two minutes.
...
and explicit user actions. Only whitelisted fixed-anchor names are classified as a known standard. On English-language
